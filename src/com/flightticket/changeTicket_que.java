...
public class changeTicket_que {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					changeTicket_que window = new changeTicket_que(args);
	...
	public changeTicket_que(String[] args) {
		initialize(args);
	}
	...
	private void initialize(String[] args) {
		...
		JButton button = new JButton("确认改签");
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (result>0) JOptionPane.showMessageDialog(null,"您的订单已记录到数据库中。","成功！",JOptionPane.INFORMATION_MESSAGE);
					else JOptionPane.showMessageDialog(null,"插入失败。","错误！",JOptionPane.ERROR_MESSAGE);
					frame.dispose();
					changeTicket.frame.dispose();
        		} catch (Exception e3) {
        			e3.printStackTrace();
        		}
           }
